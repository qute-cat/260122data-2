import streamlit as st
import pandas as pd
import plotly.express as px

# -------------------------------------------------
# 1. 페이지 설정
# -------------------------------------------------
st.set_page_config(
    page_title="AI Agent 트렌드 이해",
    layout="wide"
)

st.title("🤖 AI Agent 유형별 트렌드 이해")
st.subheader("고3·대학생 대상 진로·전공 탐색 특강")

st.markdown("""
이 대시는 **AI Agent가 어떤 역할로 발전하고 있는지**를 살펴보고,  
👉 *앞으로 어떤 전공과 역량이 중요해질지* 함께 생각해보기 위한 특강 자료입니다.
""")

# -------------------------------------------------
# 2. 예시 데이터 (CSV 사용 ❌)
# -------------------------------------------------
df = pd.DataFrame({
    "연도": [
        2020, 2020,
        2021, 2021,
        2022, 2022,
        2023, 2023,
        2024, 2024
    ],
    "AI_Agent_유형": [
        "Task-oriented Agent", "Conversational Agent",
        "Task-oriented Agent", "Conversational Agent",
        "Conversational Agent", "Autonomous Agent",
        "Autonomous Agent", "Multi-Agent System",
        "Multi-Agent System", "Autonomous Agent"
    ]
})

# -------------------------------------------------
# 3. AI Agent 유형별 빈도
# -------------------------------------------------
st.header("① AI Agent 유형별 등장 빈도")

type_counts = (
    df["AI_Agent_유형"]
    .value_counts()
    .reset_index()
    .rename(columns={
        "index": "AI Agent 유형",
        "AI_Agent_유형": "등장 빈도"
    })
)

fig1 = px.bar(
    type_counts,
    x="AI Agent 유형",
    y="등장 빈도",
    title="AI Agent 유형별 등장 빈도",
    range_y=[0, 20]
)

st.plotly_chart(fig1, use_container_width=True)

# -------------------------------------------------
# 4. 학생 대상 해석
# -------------------------------------------------
st.markdown("""
### 🧠 해석 포인트 (학생 눈높이)

- **Task-oriented Agent**
  - 정해진 일을 대신 처리하는 AI
  - 예: 챗봇, 일정 관리, 과제 보조

- **Conversational Agent**
  - 사람과 자연스럽게 대화하는 AI
  - 예: 상담 챗봇, 고객 응대 AI

📌 **의미**
> 지금까지의 AI는  
> **‘스스로 판단하는 존재’라기보다  
> ‘사람이 시킨 일을 잘 수행하는 도구’에 가까웠습니다.**
""")

# -------------------------------------------------
# 5. 연도별 트렌드 변화
# -------------------------------------------------
st.header("② 연도별 AI Agent 트렌드 변화")

trend = (
    df
    .groupby(["연도", "AI_Agent_유형"])
    .size()
    .reset_index(name="건수")
)

fig2 = px.line(
    trend,
    x="연도",
    y="건수",
    color="AI_Agent_유형",
    markers=True,
    title="연도별 AI Agent 유형 변화",
    range_y=[0, 20]
)

st.plotly_chart(fig2, use_container_width=True)

# -------------------------------------------------
# 6. 트렌드 해석
# -------------------------------------------------
st.markdown("""
### 🔍 트렌드 해석 (진로 연결)

- 최근으로 갈수록:
  - **Autonomous Agent**
  - **Multi-Agent System**
  이 점점 증가하고 있습니다.

📌 이것은 무엇을 의미할까요?

> AI가  
> **‘시키는 일만 수행하는 단계’를 넘어  
> ‘상황을 이해하고 스스로 행동하는 단계’로 이동 중**이라는 뜻입니다.
""")

# -------------------------------------------------
# 7. 진로·전공 탐색 시사점
# -------------------------------------------------
st.header("③ 진로·전공 탐색에 주는 메시지")

st.markdown("""
### 🎯 앞으로 중요해질 역량

✔ 단순 코딩 능력 → 한계  
✔ **문제를 정의하고 AI의 역할을 설계하는 능력** → 핵심

#### 연관 전공 예시
- 컴퓨터공학 / 인공지능
- 심리학 / 인지과학
- 산업공학 / UX·서비스 기획
- 교육 / 상담 / 정책

📌 **정리**
> AI 시대의 경쟁력은  
> **기술 + 인간 이해 + 문제 해결 능력의 결합**입니다.
""")

# -------------------------------------------------
# 8. 특강 마무리 메시지
# -------------------------------------------------
st.success("""
🎓 오늘의 핵심 질문

👉 나는 AI를 **만드는 사람**이 되고 싶은가?  
👉 AI와 **함께 일하는 사람**이 되고 싶은가?  
👉 AI를 **활용해 문제를 해결하는 사람**이 되고 싶은가?

이 질문이 여러분의 전공 선택과 대학 생활의 출발점이 되길 바랍니다.
""")
